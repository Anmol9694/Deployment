...
class TransformerDataGenerator:
    ...
    def get_ambient_temp(self):
        try:
            ...
            querystring = {"q":"jodhpur,india"}

            headers = {
                'x-rapidapi-host': "weatherapi-com.p.rapidapi.com",
                'x-rapidapi-key': "d6228979a2msh6118735409f0657p192d0fjsn7d91fc0b41d7"
                }
    ...
    """
    def get_ambient_temp(self,T=35):
        amb_temp=T+np.random.normal(2.5,0.25,1)
        return amb_temp
    ...
    def cell_temperature(self,solar_irr,amb_temp,cell_temp_model="sapm",rack='insulated_back_glass_polymer',wind_speed=1,noise_c_temp_per=(0.9,0.03)):
        try:
    ...
    def module_power_output(self,solar_irradiance,cell_temp,pdc0=240, gamma_pdc=-0.005, temp_ref=25.0):
        try:
            module_power_output=pvlib.pvsystem.pvwatts_dc(g_poa_effective=solar_irradiance, temp_cell=cell_temp, pdc0=pdc0, gamma_pdc=gamma_pdc, temp_ref=temp_ref)
    ...
    def dc_current_voltage(self,solar_irradiance,cell_temp,parameters = {
        ...
        dEgdT=-0.0002677,    ivcurve_pnts=100,
        method='lambertw',noise_dc_c_v_per=(0.9,0.03)):
    ...
    ....
    """
    def inv_config_retrieve(self,name1='ABB__MICRO_0_25_I_OUTD_US_208__208V_',model1='CECInverter',name2='ABB__MICRO_0_25_I_OUTD_US_240__240V_',model2='CECInverter'):
        try:
    ...
    def ac_power_current(self,v_dc, p_dc, inverter1,inverter2,noise_ac_p_c=(5.5,1.5)):
        ...
        try:
            ...
            ac_power1=(Paco1 / (A1 - B1) - C1 * (A1 - B1)) * (p_dc - B1) + C1 * (p_dc - B1)**2+np.random.normal(noise_ac_p_c[0],noise_ac_p_c[1],1)
            ac_current_lv_w1=ac_power1/float(inverter1['Vac'])+np.random.normal(noise_ac_p_c[0],noise_ac_p_c[1],1)
            ...
            ac_power2=(Paco2 / (A2 - B2) - C2 * (A2 - B2)) * (p_dc - B2) + C2 * (p_dc - B2)**2+np.random.normal(noise_ac_p_c[0],noise_ac_p_c[1],1)
            ac_current_lv_w2=ac_power2/float(inverter2['Vac'])+np.random.normal(noise_ac_p_c[0],noise_ac_p_c[1],1)
    ...
    def current_hv(self,ac_current_lv_w1,ac_current_lv_w2,turns_ratio=(10,1),noise_c_hv_per=(0.85,0.05)):
        try:
            current_hv=(ac_current_lv_w1+ac_current_lv_w2)*turns_ratio[1]/turns_ratio[0]*np.random.normal(noise_c_hv_per[0],noise_c_hv_per[1],1)
    ...
    def power_lv1_lv2_hv(self,ac_current_lv_w1,ac_current_lv_w2,ac_current_hv,Rp1,Rp2,Rs,noise_p_lv1_lv2_hv=(5.5,1.5)):
    ...
    def temp_lv1_lv2_hv(self,amb_temp,Pp1,Pp2,Ps,area=7350,noise_t_lv1_lv2_hv=(6.5,2)):
        ...
        temp_lv_w1=(Pp1/7350)**0.833+amb_temp+np.random.normal(noise_t_lv1_lv2_hv[0],noise_t_lv1_lv2_hv[1],1)

        temp_lv_w2=(Pp2/7350)**0.833+amb_temp+np.random.normal(noise_t_lv1_lv2_hv[0],noise_t_lv1_lv2_hv[1],1)
    ...
    def data_generator(self):
        ...
        ac_power1,ac_power2,ac_lv_w1,ac_lv_w2=self.ac_power_current(dc_current_voltage[1],module_power,inv1,inv2)
        ...
        tf1_arr_pred=model.predict(tf1_arr_transform)
        ...
        tf1_arr_pred_inv=scaler.inverse_transform(tf1_arr_pred_reshaped)+np.random.normal(1.5,3.5,len(tf1_arr_pred))
        print(tf1_arr_pred_inv)
        
        columns_pred=['TEMPERATURE LV W1 PREDICTED', 'TEMPERATURE LV W2 PREDICTED', 'TEMPERATURE HV PREDICTED', 'OTI TEMPERATURE PREDICTED']
        ...
        trans_dict=tf1_c.to_dict(orient='records')
